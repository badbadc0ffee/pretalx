...
- :feature:`dev` Plugins can now inject content into pages in the organisers area related to persons, mails and submissions with the ``pretalx.person.signals.person_details``, ``pretalx.mail.signals.mail_badge``, ``pretalx.mail.signals.mail_details``, ``pretalx.submission.signals.submission_details`` and ``pretalx.submission.signals.submission_links`` signals.
